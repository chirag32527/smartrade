...
def get_PL_Data(aurl,aname):
	global pl_already_exist
	if(aname.strip() not in pl_already_exist):
		print "------------------------------------",aname,"----------------------------------------------------------------------"
		get_Data(aurl,aname+"-PL-")
	else:
		print aname," already exists"
	return	


def get_BS_Data(aurl,aname):
	global bs_already_exist
	if(aname.strip() not in bs_already_exist):
		rows = get_Data(aurl,aname+"-BS-")
	else:
		print aname," already exists"
	return
	
def get_Yearly_Data(aurl,aname):
	global yearly_already_exist
	if(aname.strip() not in yearly_already_exist):
		rows=get_Data(aurl,aname+"-Yearly-")
	else:
		print aname," already exists"
	return
		
def get_Quaterly_Data(aurl,aname):
	global quaterly_already_exist
	if(aname.strip() not in quaterly_already_exist):
		rows=get_Data(aurl,aname+"-Quaterly-")
	else:
		print aname," already exists"
	return
...
def get_Ratio_Data(aurl,aname):
	global ratios_already_exist
	if(aname.strip() not in ratios_already_exist):
		rows=get_Data(aurl,aname+"-Ratios-")
	else:
		print aname," already exists"
	return
	...
	if index != -1:
		...
		for field in fields:
			...
			if field.get_text()=="Ratios":
				required_link = baseurl+field['href']
				get_Ratio_Data(required_link,aname)
			
			if field.get_text()=="Yearly Results":
				required_link=baseurl + field['href']
				get_Yearly_Data(required_link,aname)
			
			if field.get_text()=="Quarterly Results":
				required_link=baseurl+field['href']
				get_Quaterly_Data(required_link,aname)
...
os.chdir("/media/chirag/7670-4A22/output/Companies")
pl_already_exist=[]
for f in glob.glob("*-PL-*.csv"):
	pl_already_exist.append(f.split("-")[0])

bs_already_exist=[]
for f in glob.glob("*-BS-*.csv"):
	bs_already_exist.append(f.split("-")[0])

yearly_already_exist=[]
for f in glob.glob("*-Yearly-*.csv"):
	yearly_already_exist.append(f.split("-")[0])
	
quaterly_already_exist=[]
for f in glob.glob("*-Quaterly-*.csv"):
	quaterly_already_exist.append(f.split("-")[0])

ratios_already_exist=[]
for f in glob.glob("*-Ratios-*.csv"):
	ratios_already_exist.append(f.split("-")[0])
